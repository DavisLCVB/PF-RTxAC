...
def calculate_network_base(edificios, pisos_maximo, pcs_por_piso, ip_base_str, ip_base_edificios):
    ...
    bits_pcs = math.ceil(
        math.log2(pcs_por_piso + 2 + 1))  # +2 por dirección de red misma y broadcast, y +1 por el router de la subred
...
def assign_routers_and_pisos(subred_edificios, conexiones_router):
    ...
    for i, conexion in enumerate(conexiones_router):
        conexion_ips[f"Conexión {i + 1}"] = [conexion[0].network_address + 1,
                                             conexion[0].network_address + 2]  # /30 tiene 2 direcciones utilizables
...
try:
    ...
    result = calculate_network_base(edificios=len(pisos_por_edificio), pisos_maximo=max(pisos_por_edificio),
                                    pcs_por_piso=30, ip_base_str="192.162.0.0", ip_base_edificios="172.16.10.0")
    ...
    subred_edificios, subredes_pisos, conexiones_router = create_subnets_for_routers(result["ip_base"],
                                                                                     result["bits_edificios"],
                                                                                     result["bits_pisos"],
                                                                                     pisos_por_edificio)
